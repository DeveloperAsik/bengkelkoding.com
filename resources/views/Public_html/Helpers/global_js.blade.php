<script>
    ...
    var str_slug = function (string) {
        var str = string.replace(/\+/g, ''); str = str.replace(/:/g, ''); str = str.replace(/,/g, ''); str = str.replace(/\./g, ''); str = str.replace(/&/g, ''); str = str.replace(/\|/g, ''); str = str.replace(/ /g, '-'); str = str.replace(/\--/g, '-'); str = str.replace(/\---/g, '-'); return str.toLowerCase();
    };
    var fnSetPaginationInfo = function (el, data) {
        if (el) { for (var i = 0; i < Object.keys(data).length; i++) { var key = Object.keys(data)[i]; $(el).attr(key, data[key]); } }
    };
    ...
    var fnFetchApi = function (params) {
        if (params) {
            var settings = {
                url: params.url,
    ...
    var loadingBg = function (action, el = '', msg = '', response) {
        if (action === 'start' && el == '') { console.log('load bg'); $('body').append('<div id="loading_bg" style="z-index:9998; background-color:#f9f9fb; opacity:0.8; width:100%; height:100%; position: fixed; top:0px; left:0px"><button style="position:fixed; top: 40%; left: 50%; background-color: transparent; border: none; color: #fff;" class="btn btn-primary btn-loading btn-xl btn-svg-icon"><svg width="16px" height="16px"><use xlink:href="' + _base_assets_url + '/images/loading/sprite.svg#quickview-16"></use></svg></button></div>'); } else { if (el) { var color = 'rgb(204, 51, 51)'; if (response === 'success') { color = 'rgb(52, 194, 105)'; } $(el).html('<p style="color:' + color + '">' + msg + '</p>'); } $('#loading_bg').remove(); }
    };
    var loadingImg = function (el, act, opt) {
        if (!el) el = 'img-loading';
        if (!opt) opt = 'circle-loading';
        var animation = bodymovin.loadAnimation({container: document.getElementById(el), path: _path_json + '/lottie/' + opt + '.json', renderer: 'svg', loop: true, autoplay: true, name: "Loading cuy... sabar ye" }); if (act === 'play' || act == 'start') { loadingBg('start'); $('#' + el).show(); animation.play(); } else if (act === 'destroy' || act === 'stop') { loadingBg('stop'); animation.destroy(); $('#' + el).hide(); }
    };
